...
- [6. 🆚 int vs long vs long long 在不同平台上有何差异？](#6--int-vs-long-vs-long-long-在不同平台上有何差异)
- [7. 🤔 void 类型有什么作用？](#7--void-类型有什么作用)
- [8. 🤔 什么是 typedef？它如何创建类型别名？](#8--什么是-typedef它如何创建类型别名)
- [9. 🤔 字符类型 char 是有符号还是无符号？](#9--字符类型-char-是有符号还是无符号)
- [10. 🆚 float vs double vs long double 的精度与存储有何区别？](#10--float-vs-double-vs-long-double-的精度与存储有何区别)
...
## 6. 🆚 int vs long vs long long 在不同平台上有何差异？
...
- 函数返回类型为 `void`：
  - 表示函数没有返回值，例如 `void foo` 只负责执行操作。
- 指针类型 `void*`：
  - 表示指向某种未知类型的指针，常用于通用接口和内存管理函数中。
  - 使用前需要显式转换成具体类型指针。
- 函数参数为 `void`：
  - 在函数声明中写作 `void` 表示函数不接受任何参数。
...
- 如果将 `char` 用于存放纯二进制数据，如图像、音频等，在不同平台上它可能被当成有符号或无符号处理。
...
## 10. 🆚 float vs double vs long double 的精度与存储有何区别？
...
`float` 占用空间小，但精度有限，容易累积误差；`double` 是多数数值算法的默认选择；而 `long double` 是否比 `double` 更精确，需要查阅具体编译器文档。
